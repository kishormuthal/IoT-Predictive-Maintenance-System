...
def example_adaptive_thresholding():
    ...
    gev_result = AdaptiveThresholdCalculator.gev_threshold(normal_data, confidence_level=0.99, block_size=100)
    ...
    pot_result = AdaptiveThresholdCalculator.pot_threshold(normal_data, confidence_level=0.99)
    ...
    mad_result = AdaptiveThresholdCalculator.mad_threshold(normal_data, confidence_level=0.99)
    ...
    iso_result = AdaptiveThresholdCalculator.isolation_forest_threshold(normal_data, contamination=0.01)
...
def example_probabilistic_scoring():
    ...
    for test_value in test_values:
        ...
        gaussian_score = ProbabilisticAnomalyScorer.gaussian_likelihood_score(test_value, reference_data, alpha=0.05)
        print(f"Gaussian Score: {gaussian_score.score:.4f} " f"(prob={gaussian_score.probability:.4f})")
        ...
        print(f"Bayesian Score: {bayesian_score.score:.4f} " f"(P(anomaly|data)={bayesian_score.probability:.4f})")

        # Method 3: KDE Score
        kde_score = ProbabilisticAnomalyScorer.kernel_density_score(test_value, reference_data)
        print(f"KDE Score: {kde_score.score:.4f} " f"(prob={kde_score.probability:.4f})")
        ...
        print(f"Ensemble Score: {ensemble_score.score:.4f} " f"(prob={ensemble_score.probability:.4f})")
    ...
    print(f"95% CI: [{confidence['confidence_interval'][0]:.4f}, " f"{confidence['confidence_interval'][1]:.4f}]")
...
def example_advanced_imputation():
    ...
    spline_imputed = AdvancedImputer.spline_interpolation(data_with_missing, order=3, smoothing=0.1)
    ...
    print("\n--- KNN Imputation ---")
    knn_imputed = AdvancedImputer.knn_imputation(data_with_missing, n_neighbors=5, weights="distance")
    ...
    adaptive_imputed = AdvancedImputer.adaptive_imputation(data_with_missing, method="auto")
    ...
    imputed_mean, imputed_std = AdvancedImputer.impute_with_confidence(data_with_missing, method="auto", n_bootstrap=50)
...
def example_ensemble_methods():
    ...
    perf_result = EnsembleAggregator.performance_weighted_average(model_predictions, model_performance)
    ...
    var_result = EnsembleAggregator.inverse_variance_weighting(model_predictions, model_variances)
    ...
    trimmed_result = EnsembleAggregator.trimmed_mean(model_predictions, trim_percent=0.2)
