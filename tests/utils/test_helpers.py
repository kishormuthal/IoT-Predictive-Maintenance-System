...
class MemoryMonitor:
    ...
    def get_memory_usage(self) -> Dict[str, float]:
        ...
        return {
            ...
            "increase_mb": self.final_memory - self.initial_memory if self.final_memory and self.initial_memory else 0,
...
class TestDataValidator:
    ...
    def validate_anomaly_data(anomalies: List[Dict[str, Any]]) -> Dict[str, bool]:
        ...
        results["all_have_required_fields"] = all(
            all(field in anomaly for field in required_fields)
            for anomaly in anomalies
        )

        # Check score ranges (0-1)
        results["scores_in_range"] = all(
            0.0 <= anomaly.get("score", -1) <= 1.0
            for anomaly in anomalies
        )
        ...
        results["valid_severities"] = all(
            anomaly.get("severity") in valid_severities
            for anomaly in anomalies
        )
...
class PerformanceBenchmark:
    ...
    def compare_operations(self, operation1: str, operation2: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "performance_ratio": bench2["execution_time"] / bench1["execution_time"] if bench1["execution_time"] > 0 else float('inf')
        }
...
class TestSessionTracker:
    ...
    def add_test_result(self, test_name: str, test_type: str, passed: bool, execution_time: float = 0.0, error: str = None):
...
def assert_within_tolerance(actual: float, expected: float, tolerance: float = 0.01, message: str = ""):
    ...
    assert diff <= tolerance_value, f"{message} - Expected: {expected}, Actual: {actual}, Tolerance: {tolerance_value}, Diff: {diff}"
...
def create_mock_sensor_data(sensor_id: str, num_points: int = 100, base_value: float = 50.0, noise_level: float = 5.0) -> Dict[str, Any]:
