...
class TestServiceInitializationPerformance:
    ...
    def test_core_services_initialization(self, profiler):
        ...
        core_services = [
            ('AnomalyDetectionService', 'src.core.services.anomaly_service.AnomalyDetectionService'),
            ('ForecastingService', 'src.core.services.forecasting_service.ForecastingService'),
            ('NASADataLoader', 'src.infrastructure.data.nasa_data_loader.NASADataLoader')
        ]
        ...
        for service_name, service_path in core_services:
            ...
            if result['success']:
                ...
                assert result['memory_increase'] < 100, f"{service_name} uses too much memory: {result['memory_increase']}MB"
            else:
    ...
    def test_optional_services_initialization(self, profiler):
        ...
        optional_services = [
            ('TrainingUseCase', 'src.application.use_cases.training_use_case.TrainingUseCase'),
            ('TrainingConfigManager', 'src.application.services.training_config_manager.TrainingConfigManager'),
            ('ModelRegistry', 'src.infrastructure.ml.model_registry.ModelRegistry'),
            ('PerformanceMonitor', 'src.infrastructure.monitoring.performance_monitor.PerformanceMonitor')
        ]
    ...
    def test_nasa_data_loader_performance(self, profiler):
        ...
        result = profiler.profile_service_init('NASADataLoader_Detailed',
        ...
        if result['success']:
            ...
            assert result['memory_increase'] < 200, f"NASA Data Loader uses too much memory: {result['memory_increase']}MB"
    ...
    def test_service_initialization_bottleneck_analysis(self, profiler):
        ...
        print(f"Most memory: {summary['most_memory_service']['name']} ({summary['most_memory_service']['memory']:.2f}MB)")

        # Performance assertions
        assert summary['average_init_time'] < 5.0, f"Average service init time too high: {summary['average_init_time']:.2f}s"
        assert summary['total_memory_increase'] < 500, f"Total memory usage too high: {summary['total_memory_increase']:.2f}MB"
