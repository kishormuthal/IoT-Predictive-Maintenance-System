...
from src.anomaly_detection.models.lstm_autoencoder import LSTMAutoencoder
...
class ModelTrainer:
    ...
    def train_lstm_autoencoder(self, X_train, X_val, hyperparams=None):
        ...
        history = model.model.fit(
            X_train,
            X_train,  # Input and output are the same
    ...
    def evaluate_anomaly_model(self, model, X_test, y_test, model_name):
        ...
        # Calculate metrics
        metrics = AnomalyDetectionMetrics.calculate_metrics(y_test, predictions)
        ...
        return {"metrics": metrics.to_dict(), "classification_report": report}
