...
def test_imports(result: TestResult):
    ...
    try:
        from src.presentation.dashboard.enhanced_app_optimized import OptimizedIoTDashboard, create_app
...
def test_configuration(result: TestResult):
    ...
    try:
        ...
        if len(equipment_list) == 12:
            ...
            result.add_test("Equipment config", True, f"{len(equipment_list)} equipment units", warning=True)
    except Exception as e:
...
def test_data_access(result: TestResult):
    ...
    try:
        ...
        try:
            ...
            result.add_test("Load SMAP data", True, "Data loading error (will use mock)", warning=True)
