        ...
        for dataset_name, data in self.nasa_datasets.items():
            if isinstance(data, np.ndarray):
                try:
                    ...
                    logger.info(f"{status} {dataset_name}: {data.shape}, {validation_results[dataset_name]['size_mb']:.2f}MB")
        ...
        try:
            ...
            if 'msl_train' in self.nasa_datasets:
                ...
                if hasattr(detector, 'detect_anomalies'):
                    ...
                    logger.info(f"✓ MSL anomaly detection: {pipeline_results['msl']['anomalies_detected']} anomalies in {detection_time:.4f}s")
            ...
            if 'smap_train' in self.nasa_datasets:
                ...
                if hasattr(detector, 'detect_anomalies'):
                    ...
                    logger.info(f"✓ SMAP anomaly detection: {pipeline_results['smap']['anomalies_detected']} anomalies in {detection_time:.4f}s")
    ...
    async def validate_performance_benchmarks(self):
        ...
        if self.nasa_datasets:
            ...
            if largest_dataset is not None:
                ...
                logger.info(f"Data processing performance: {performance_results['data_processing']['throughput']:.2f} samples/s")
