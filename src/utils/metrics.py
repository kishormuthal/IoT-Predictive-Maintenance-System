    ...
    def calculate_metrics(y_true: np.ndarray, y_pred: np.ndarray, multivariate: bool = False) -> ForecastMetrics:
        ...
        try:
            ...
            mse = mean_squared_error(
                y_true,
                y_pred,
                multioutput="uniform_average" if multivariate else "uniform_average",
            )
            rmse = np.sqrt(mse)
...
class MaintenanceSchedulingMetrics:
    ...
    def calculate_metrics(
        ...
        try:
            ...
            preventive_count = (
                len(period_orders[period_orders["maintenance_type"] == "PREVENTIVE"])
                if "maintenance_type" in period_orders.columns
                else 0
            )
    ...
    @staticmethod
    def calculate_equipment_reliability(
        equipment_df: pd.DataFrame,
        work_orders_df: pd.DataFrame,
