...
from .components.time_controls import TimeControlManager, TimeRange, TimeControlConfig
from .components.alert_system import AlertManager, AlertSeverity, AlertCategory
...
class OptimizedIoTDashboard:
    ...
    def __init__(self, debug: bool = False):
        ...
        self.event_bus.register_component('dashboard_core', {
            'type': 'main_dashboard',
    ...
    def _initialize_services_optimized(self):
        ...
        try:
            ...
            self.anomaly_service = safe_service_init(AnomalyDetectionService, "Anomaly service", 8) or AnomalyDetectionService()
            self.forecasting_service = safe_service_init(ForecastingService, "Forecasting service", 8) or ForecastingService()
        ...
        try:
            ...
            self.event_bus.emit(
                ...
                'dashboard_core',
                {
                    ...
                    'transformer_available': availability_report['availability_summary'].get('transformer_available', 0),
                    'coverage_percentage': availability_report['availability_summary'].get('coverage_percentage', 0)
                }
            )

            logger.info(f"Model availability updated: {availability_report['availability_summary'].get('coverage_percentage', 0):.1f}% coverage")
    ...
    def _setup_optimized_layout(self):
        ...
        header = dbc.Container([
            dbc.Row([
                dbc.Col([
                    ...
                    html.P([
                        html.Span("Optimized Mode", className="badge bg-success me-2"),
                        html.Span("Full Features", className="badge bg-primary me-2"),
        ...
        nav_tabs = html.Div([
            ...
            ], className="nav nav-tabs", id="optimized-tabs"),
            html.Div(id="active-tab-indicator", style={"display": "none"}, children="overview")
        ], className="mb-3")
    ...
    def _setup_optimized_callbacks(self):
                ...
                return [
                    dbc.Card([
                        dbc.CardBody([
                            ...
                            html.P("Operational", className="mb-0 text-success fw-bold"),
        ...
        def handle_tab_navigation(overview_clicks, monitoring_clicks, anomalies_clicks,
                                forecasting_clicks, maintenance_clicks, work_orders_clicks,
            ...
            try:
                ...
                tab_classes = []
                for tab_name in ["overview", "monitoring", "anomalies", "forecasting", "maintenance", "work_orders", "system_performance"]:
                ...
                return ["overview", dbc.Alert(f"Error: {str(e)}", color="danger")] + ["nav-link active"] + ["nav-link"] * 6
    ...
    def _create_sample_alerts(self):
        ...
        try:
            ...
            if self.dashboard_state.get('model_availability', {}).get('availability_summary', {}).get('coverage_percentage', 0) < 50:
                self.alert_manager.create_alert(
                    "Low Model Coverage",
                    ...
                    dbc.Card([
                        ...
                        dbc.CardBody([
                            dbc.Row([
                                dbc.Col([
                                    dbc.Alert([
                                        html.H6("Telemanom (Anomaly Detection)", className="mb-1"),
                                        html.P(f"{telemanom_available}/{len(self.equipment_list)} sensors trained", className="mb-0")
                                    ], color="success" if telemanom_available > 0 else "warning")
                                ], width=6),
                                dbc.Col([
                                    dbc.Alert([
                                        html.H6("Transformer (Forecasting)", className="mb-1"),
                                        html.P(f"{transformer_available}/{len(self.equipment_list)} sensors trained", className="mb-0")
                                    ], color="info" if transformer_available > 0 else "warning")
                                ], width=6)
                            ])
                        ])
                    ])
            ...
            dbc.Row([
                dbc.Col([
                    dbc.Card([
                        ...
                        dbc.CardBody([
                            html.Div([
                                dbc.Badge(f"{eq.equipment_id}", color="primary", className="me-2 mb-2")
                                for eq in self.equipment_list if eq.equipment_id.startswith("SMAP")
                            ])
                        ])
                    ])
                ], width=6),
                dbc.Col([
                    dbc.Card([
                        ...
                        dbc.CardBody([
                            html.Div([
                                dbc.Badge(f"{eq.equipment_id}", color="success", className="me-2 mb-2")
                                for eq in self.equipment_list if eq.equipment_id.startswith("MSL")
                            ])
                        ])
                    ])
                ], width=6)
                ...
                dbc.Col([
                    dbc.Card([
                        ...
                        dbc.CardBody([
                            ...
                            html.P("Monitoring 12 sensors (6 SMAP + 6 MSL) using NASA's advanced telemetry anomaly detection"),
                            dbc.Progress(value=85, label="System Performance: 85%", className="mb-3"),
                            dbc.ButtonGroup([
                                dbc.Button("View Real-time Data", color="primary", size="sm"),
                                dbc.Button("Historical Analysis", color="secondary", size="sm"),
                                dbc.Button("Export Data", color="info", size="sm")
                            ])
                        ])
                    ])
                ], width=12)
                ...
                dbc.Col([
                    dbc.Card([
                        ...
                        dbc.CardBody([
                            ...
                            html.P("Using NASA's Telemanom LSTM-based algorithm for spacecraft telemetry anomaly detection"),
                            dbc.Row([
                                dbc.Col([
                                    html.H4("0", className="text-success"),
                                ...
                                dbc.Col([
                                    html.H4("99.8%", className="text-primary"),
                ...
                dbc.Col([
                    dbc.Card([
                        ...
                        dbc.CardBody([
                            ...
                            html.P("Advanced forecasting using Transformer models for predictive maintenance"),
                            dbc.Row([
                                dbc.Col([
                                    html.H4("24h", className="text-primary"),
                                    html.P("Forecast Horizon")
                                ], width=4),
                                dbc.Col([
                                    html.H4("95%", className="text-success"),
                                    html.P("Prediction Accuracy")
                                ], width=4),
                                dbc.Col([
                                    html.H4("Low", className="text-info"),
                                    html.P("Risk Level")
                                ], width=4)
                            ])
                        ])
