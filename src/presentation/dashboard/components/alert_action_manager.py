...
# Maintenance functionality integrated into dashboard - using mock classes for now
from src.core.models.anomaly import AnomalyResult
...
class InteractiveAlertActionManager:
    ...
    def __init__(
        ...
        work_order_manager: WorkOrderManager = None,
    ):
    ...
    def _create_action_buttons(self, alert_id: str) -> dbc.ButtonGroup:
        ...
        return dbc.ButtonGroup(
            [
                dbc.Button(
                    [html.I(className="fas fa-check me-2"), "Acknowledge"],
                    id=f"acknowledge-btn-{alert_id}",
                    color="success",
                    size="sm",
                ),
                dbc.Button(
                    [html.I(className="fas fa-wrench me-2"), "Create Work Order"],
                    id=f"work-order-btn-{alert_id}",
                    color="primary",
                    size="sm",
                ),
                dbc.Button(
                    [html.I(className="fas fa-times me-2"), "Dismiss"],
                    id=f"dismiss-btn-{alert_id}",
                    color="warning",
                    size="sm",
                ),
                dbc.Button(
                    [html.I(className="fas fa-arrow-up me-2"), "Escalate"],
                    id=f"escalate-btn-{alert_id}",
                    color="danger",
                    size="sm",
                ),
            ],
            ...
            [
                ...
                dbc.ModalFooter(
                    [
                        dbc.Button(
                            "Cancel",
                            id=f"acknowledge-cancel-{alert_id}",
                            color="secondary",
                            className="me-2",
                        ),
                        dbc.Button(
                            "Acknowledge",
                            id=f"acknowledge-confirm-{alert_id}",
                            color="success",
                        ),
                    ]
                ),
            ],
            id=f"acknowledge-modal-{alert_id}",
            is_open=False,
        )
            ...
            [
                ...
                dbc.ModalFooter(
                    [
                        dbc.Button(
                            "Cancel",
                            id=f"wo-cancel-{alert_id}",
                            color="secondary",
                            className="me-2",
                        ),
                        dbc.Button(
                            "Create Work Order",
                            id=f"wo-confirm-{alert_id}",
                            color="primary",
                        ),
                    ]
                ),
            ],
            id=f"work-order-modal-{alert_id}",
            is_open=False,
            size="lg",
        )
            ...
            [
                    ...
                    [
                        ...
                        dcc.Dropdown(
                            ...
                            options=[
                                ...
                                {
                                    "label": "Multiple related failures",
                                    "value": "pattern_detected",
                                },
                                {
                                    "label": "Critical equipment impact",
                ...
                dbc.ModalFooter(
                    [
                        dbc.Button(
                            "Cancel",
                            id=f"escalate-cancel-{alert_id}",
                            color="secondary",
                            className="me-2",
                        ),
                        dbc.Button(
                            "Escalate",
                            id=f"escalate-confirm-{alert_id}",
                            color="danger",
                        ),
                    ]
                ),
            ],
            id=f"escalate-modal-{alert_id}",
            is_open=False,
        )
        ...
        return dbc.Card(
            [
                dbc.CardHeader(
                    [
                        html.H6(
                            [html.I(className="fas fa-history me-2"), "Action History"],
                            className="mb-0",
                        )
                    ]
                ),
                dbc.CardBody([html.Div(id=f"action-history-content-{alert_id}")]),
            ],
            className="mt-3",
        )
    ...
    def create_work_order_from_alert(
        self, alert_id: str, user_id: str, work_order_params: Dict[str, Any]
    ) -> AlertActionResult:
        """
            ...
            action_items.append(
                html.Div(
                    [
                        ...
                        html.Small(
                            f" by {action.user_id} at {action.timestamp.strftime('%Y-%m-%d %H:%M:%S')}",
                            className="text-muted ms-2",
                        ),
                        html.Br(),
                        html.Small(
                            action.reason if action.reason else action.notes,
