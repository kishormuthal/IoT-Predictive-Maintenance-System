...
def create_alert_system_components():
    ...
    return html.Div([
        ...
        dbc.Modal([
            ...
            dbc.ModalBody([
                html.Div(id="alert-management-content")
            ]),
            dbc.ModalFooter([
                dbc.Button("Clear All", id="clear-all-alerts-btn", color="warning", className="me-2"),
                dbc.Button("Close", id="close-alert-modal", className="ms-auto")
            ])
        ], id="alert-management-modal", size="lg"),
        ...
        html.Div([
            ...
            ], id="alert-summary-btn", color="outline-secondary", size="sm")
        ], id="alert-summary-component"),

        # Hidden stores for alert data
        dcc.Store(id="alert-store", data=[]),
        dcc.Store(id="alert-config-store", data={
        ...
        try:
            ...
            for alert_dict in recent_alerts:
                ...
                severity_map = {
                    ...
                    AlertSeverity.WARNING: {"icon": "warning", "header_color": "warning"},
                    AlertSeverity.ERROR: {"icon": "danger", "header_color": "danger"},
                    AlertSeverity.CRITICAL: {"icon": "danger", "header_color": "danger"}
                }
                ...
                toast = dbc.Toast([
                    ...
                    html.Div([
                        html.Small(f"Source: {alert.source}", className="text-muted me-3"),
                        html.Small(f"Category: {alert.category.value.title()}", className="text-muted")
                    ], className="mb-2"),
                    html.Div(action_buttons) if action_buttons else None,
                    html.Div([
                        dbc.Button("Acknowledge", id=f"ack-alert-{alert.id}", size="sm", color="success", className="me-1"),
                        dbc.Button("Dismiss", id=f"dismiss-alert-{alert.id}", size="sm", color="secondary")
                    ], className="mt-2")
                ],
                ...
                alert_item = dbc.ListGroupItem([
                    dbc.Row([
                        dbc.Col([
                            ...
                            html.P(alert.message, className="mb-2"),
                            html.Div([
                                dbc.Badge(alert.severity.value.upper(),
                                         color=severity_colors.get(alert.severity, "secondary"),
                                         className="me-2"),
                                dbc.Badge(alert.category.value.upper(),
                                         color=category_colors.get(alert.category, "secondary"),
                                         className="me-2"),
                                html.Small(f"Source: {alert.source}", className="text-muted")
                            ])
                        ], width=8),
                        dbc.Col([
                            ...
                            html.Br(),
                            dbc.ButtonGroup([
                                dbc.Button("Ack", id=f"modal-ack-{alert.id}", size="sm", color="success"),
                                dbc.Button("Dismiss", id=f"modal-dismiss-{alert.id}", size="sm", color="secondary")
...
def _generate_system_alerts(performance_monitor, training_use_case, anomaly_service):
    ...
    try:
        ...
        untrained_count = sum(1 for status in equipment_status.values()
                            if not status.get('anomaly_detection', {}).get('trained') and
                               not status.get('forecasting', {}).get('trained'))
        ...
        if untrained_count > 0:
            alert_manager.create_alert(
                ...
                actions=[
                    {"id": "train_all", "label": "Train All Models"}
                ]
            )

        # Anomaly alerts
        anomaly_summary = anomaly_service.get_detection_summary()
        critical_anomalies = [a for a in anomaly_summary.get('recent_anomalies', [])
