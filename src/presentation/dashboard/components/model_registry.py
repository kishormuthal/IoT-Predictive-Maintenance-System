            ...
            dbc.Col([
                dbc.Card([
                    dbc.CardBody([
                        ...
                        html.P("Browse, manage, and analyze trained models across all equipment sensors.",
        ...
        dbc.Row([
            dbc.Col([
                dbc.Card([
                    ...
                    dbc.CardBody([
                        html.Div(id="registry-stats-cards")
                    ])
                ])
            ])
        ], className="mb-4"),
                ...
                dbc.Card([
                    ...
                    dbc.CardBody([
                        dbc.Row([
                            dbc.Col([
                                ...
                                dcc.Dropdown(
                                    ...
                                    options=[
                                        ...
                                        {'label': 'Anomaly Detection (Telemanom)', 'value': 'telemanom'},
                                        {'label': 'Forecasting (Transformer)', 'value': 'transformer'}
                                    ],
                                    value='all'
                                )
                            ], width=3),
                            dbc.Col([
                                html.Label("Performance Grade:", className="fw-bold"),
                                dcc.Dropdown(
                            ...
                            dbc.Col([
                                html.Label("Equipment Type:", className="fw-bold"),
                                dcc.Dropdown(
                            ...
                            dbc.Col([
                                html.Label("Search Sensor:", className="fw-bold"),
                                dcc.Dropdown(
                                    id="sensor-search",
                    ...
                    dbc.CardHeader([
                        ...
                        dbc.ButtonGroup([
                            dbc.Button([
                                ...
                                "Refresh"
                            ], id="refresh-models-btn", color="outline-primary", size="sm"),
                            dbc.Button([
                                html.I(className="fas fa-download me-1"),
                                "Export"
                            ], id="export-models-btn", color="outline-success", size="sm")
                        ], className="ms-auto")
                    ]),
                    dbc.CardBody([
        ...
        dbc.Row([
            dbc.Col([
                dbc.Card([
                    ...
                    dbc.CardBody([
                        html.Div(id="model-details-panel")
                    ])
                ])
            ], width=8),
            dbc.Col([
                dbc.Card([
                    ...
                    dbc.CardBody([
                        html.Div(id="model-actions-panel")
                    ])
                ])
            ], width=4)
        ], className="mb-4"),
        ...
        dbc.Row([
            dbc.Col([
                dbc.Card([
                    ...
                    dbc.CardBody([
                        dbc.Tabs([
                            dbc.Tab(label="Performance Distribution", tab_id="perf-dist"),
                            dbc.Tab(label="Model Versions Timeline", tab_id="versions-timeline"),
                            dbc.Tab(label="Training Metrics", tab_id="training-metrics")
                        ], id="analytics-tabs", active_tab="perf-dist"),
...
def register_model_registry_callbacks(app, model_registry, training_use_case, config_manager):
    ...
    def update_registry_statistics(n):
        ...
        try:
            ...
            # Calculate coverage
            anomaly_coverage = sum(1 for s in equipment_status.values()
                                 if s.get('anomaly_detection', {}).get('trained', False))
            forecast_coverage = sum(1 for s in equipment_status.values()
            ...
            stats_cards = dbc.Row([
                dbc.Col([
                    dbc.Card([
                        dbc.CardBody([
                            html.H3(str(total_models), className="text-primary mb-1"),
                            html.P("Total Models", className="text-muted mb-0"),
                            html.Small(f"{total_versions} versions", className="text-muted")
                        ])
                ...
                dbc.Col([
                    dbc.Card([
                        dbc.CardBody([
                            html.H3(f"{total_size_mb:.1f} MB", className="text-info mb-1"),
                            html.P("Storage Used", className="text-muted mb-0"),
                            html.Small(f"Avg: {total_size_mb/max(total_models, 1):.1f} MB/model", className="text-muted")
                        ])
                    ], className="text-center h-100")
                ], width=2),
                dbc.Col([
                    dbc.Card([
                        dbc.CardBody([
                            html.H3(f"{anomaly_coverage}/{total_equipment}", className="text-danger mb-1"),
                            html.P("Anomaly Models", className="text-muted mb-0"),
                            dbc.Progress(value=(anomaly_coverage/max(total_equipment, 1))*100,
                ...
                dbc.Col([
                    dbc.Card([
                        dbc.CardBody([
                            html.H3(f"{forecast_coverage}/{total_equipment}", className="text-success mb-1"),
                            html.P("Forecast Models", className="text-muted mb-0"),
                            dbc.Progress(value=(forecast_coverage/max(total_equipment, 1))*100,
                ...
                dbc.Col([
                    dbc.Card([
                        dbc.CardBody([
                            html.H3(f"{model_types.get('telemanom', 0)}", className="text-warning mb-1"),
                            html.P("Telemanom", className="text-muted mb-0"),
                            html.Small("Anomaly Detection", className="text-muted")
                        ])
                    ], className="text-center h-100")
                ], width=2),
                dbc.Col([
                    dbc.Card([
                        dbc.CardBody([
                            html.H3(f"{model_types.get('transformer', 0)}", className="text-secondary mb-1"),
                            html.P("Transformer", className="text-muted mb-0"),
    ...
    def update_models_table(refresh_clicks, model_type_filter, grade_filter,
                          equipment_filter, sensor_filter, n):
        ...
        try:
            ...
            # Create table
            if not models_data:
                return dbc.Alert("No models found matching the selected filters.", color="info"), []
            ...
            table = dash_table.DataTable(
                ...
                columns=[
                    ...
                    {'name': 'Performance', 'id': 'performance_score', 'type': 'numeric', 'format': {'specifier': '.3f'}},
                    {'name': 'Grade', 'id': 'performance_grade'},
                    {'name': 'Size (MB)', 'id': 'model_size_mb', 'type': 'numeric', 'format': {'specifier': '.1f'}},
    ...
    def update_model_details(selected_rows, models_data):
        ...
        try:
            if not selected_rows or not models_data:
                return "Select a model to view details", "Select a model to see available actions"
            ...
            details = dbc.ListGroup([
                dbc.ListGroupItem([
                    html.Strong("Model ID: "), metadata.model_id
                ]),
                dbc.ListGroupItem([
                    html.Strong("Version ID: "), metadata.version
                ]),
                dbc.ListGroupItem([
                    html.Strong("Created: "), metadata.created_at
                ]),
                dbc.ListGroupItem([
                    html.Strong("Training Time: "), f"{metadata.training_time_seconds:.1f} seconds"
                ]),
                dbc.ListGroupItem([
                    html.Strong("Model Size: "), f"{metadata.model_size_bytes / (1024*1024):.1f} MB"
                ]),
                dbc.ListGroupItem([
                    html.Strong("Performance Score: "), f"{metadata.performance_score:.3f}"
                ]),
                dbc.ListGroupItem([
                    html.Strong("Description: "), metadata.description or "No description"
                ])
            ], flush=True)
            ...
            actions = dbc.Stack([
                dbc.Button([
                    ...
                    "View Versions"
                ], id="view-versions-btn", color="primary", className="mb-2"),
                dbc.Button([
                    html.I(className="fas fa-check me-2"),
                    "Validate Model"
                ], id="validate-model-btn", color="success", className="mb-2"),
                dbc.Button([
                    html.I(className="fas fa-sync me-2"),
                    "Retrain Model"
                ], id="retrain-model-btn", color="warning", className="mb-2"),
                dbc.Button([
                    html.I(className="fas fa-download me-2"),
                    "Export Model"
                ], id="export-model-btn", color="info", className="mb-2"),
                dbc.Button([
                    html.I(className="fas fa-trash me-2"),
                    "Delete Version"
                ], id="delete-version-btn", color="danger")
            ])
    ...
    def update_analytics_content(active_tab, models_data):
        ...
        try:
            ...
            if active_tab == "perf-dist":
                ...
                fig = px.histogram(
                    df, x='performance_score', color='model_type',
                ...
                fig = px.scatter(
                    df, x='last_trained', y='performance_score',
                    color='model_type', size='model_size_mb',
                ...
                fig = px.box(
                    df, x='model_type', y='performance_score',
