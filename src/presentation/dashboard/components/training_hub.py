...
def create_training_hub_layout():
    ...
    return dbc.Container([
            ...
            dbc.Col([
                dbc.Card([
                    dbc.CardBody([
                        ...
                        html.P("Manage model training, monitor progress, and analyze training performance.",
                        ...
                        dbc.Row([
                            dbc.Col([
                                dbc.Button([
                                    ...
                                    "Train All Models"
                                ], id="train-all-btn", color="primary", size="lg", className="w-100 mb-2"),
                                html.Small("Train both anomaly detection and forecasting models for all sensors",
                            ...
                            dbc.Col([
                                dbc.Button([
                                    ...
                                    "Validate Models"
                                ], id="validate-all-btn", color="success", size="lg", className="w-100 mb-2"),
                                html.Small("Run validation tests on all trained models",
                                          className="text-muted")
                            ], width=3),
                            dbc.Col([
                                dbc.Button([
                                    ...
                                    "Training Report"
                                ], id="training-report-btn", color="info", size="lg", className="w-100 mb-2"),
                                html.Small("Generate comprehensive training performance report",
                                          className="text-muted")
                            ], width=3),
                            dbc.Col([
                                dbc.Button([
                                    ...
                                    "Configuration"
                                ], id="config-btn", color="secondary", size="lg", className="w-100 mb-2"),
                                html.Small("Manage training configurations and parameters",
                                          className="text-muted")
                            ], width=3)
                        ])
                    ])
                ...
                dbc.Card([
                    ...
                    dbc.CardBody([
                        dbc.Row([
                            ...
                            dbc.Col([
                                html.Label("Training Mode:", className="fw-bold"),
                                dcc.Dropdown(
                                    id="training-mode",
                                    options=[
                                        {'label': 'Standard Training', 'value': 'standard'},
                                        {'label': 'Quick Training', 'value': 'quick'},
                                        {'label': 'Extended Training', 'value': 'extended'}
                                    ],
                                    value='standard'
                                )
                            ], width=4)
                        ...
                        dbc.Row([
                            dbc.Col([
                                dbc.Button([
                                    html.I(className="fas fa-play me-2"),
                                    "Start Training"
                                ], id="start-individual-training", color="primary", className="me-2"),
                                dbc.Button([
                                    html.I(className="fas fa-stop me-2"),
                                    "Stop Training"
                                ], id="stop-training", color="danger", disabled=True)
                            ])
                        ])
                    ])
                ])
        ...
        dbc.Row([
            ...
            dbc.Col([
                dbc.Card([
                    ...
                    dbc.CardBody([
                        html.Div(id="training-logs-display",
        ...
        dcc.Interval(id='training-progress-interval', interval=2000, n_intervals=0, disabled=True)
    ])
...
def register_training_hub_callbacks(app, training_use_case, config_manager, performance_monitor):
    ...
    def update_training_status_overview(n):
        ...
        try:
            ...
            anomaly_trained = sum(1 for status in equipment_status.values()
                                if status.get('anomaly_detection', {}).get('trained', False))
            forecast_trained = sum(1 for status in equipment_status.values()
                                 if status.get('forecasting', {}).get('trained', False))

            # Calculate average performance scores
            anomaly_scores = [status.get('anomaly_detection', {}).get('performance_score', 0)
                            for status in equipment_status.values()
                            if status.get('anomaly_detection', {}).get('trained', False)]
            forecast_scores = [status.get('forecasting', {}).get('performance_score', 0)
                             for status in equipment_status.values()
                             if status.get('forecasting', {}).get('trained', False)]
            ...
            overview_cards = dbc.Row([
                dbc.Col([
                    dbc.Card([
                        dbc.CardBody([
                            html.H4(f"{anomaly_trained}/{total_equipment}", className="text-success mb-1"),
                            html.P("Anomaly Models", className="text-muted mb-0"),
                            dbc.Progress(value=(anomaly_trained/total_equipment)*100 if total_equipment > 0 else 0,
                ...
                dbc.Col([
                    dbc.Card([
                        dbc.CardBody([
                            html.H4(f"{forecast_trained}/{total_equipment}", className="text-info mb-1"),
                            html.P("Forecast Models", className="text-muted mb-0"),
                            dbc.Progress(value=(forecast_trained/total_equipment)*100 if total_equipment > 0 else 0,
                ...
                dbc.Col([
                    dbc.Card([
                        dbc.CardBody([
                            html.H4(f"{avg_anomaly_score:.2f}", className="text-primary mb-1"),
                            html.P("Avg Anomaly Score", className="text-muted mb-0"),
                            dbc.Progress(value=avg_anomaly_score*100, color="primary", className="mt-2")
                        ])
                    ], className="text-center")
                ], width=3),
                dbc.Col([
                    dbc.Card([
                        dbc.CardBody([
                            html.H4(f"{avg_forecast_score:.2f}", className="text-warning mb-1"),
                            html.P("Avg Forecast Score", className="text-muted mb-0"),
                            dbc.Progress(value=avg_forecast_score*100, color="warning", className="mt-2")
                        ])
                    ], className="text-center")
                ], width=3)
            ])
            ...
            if sensors_needing_training:
                recommendation_items.append(
                    dbc.ListGroupItem([
                        ...
                        html.P([eq['sensor_id'] for eq in sensors_needing_training[:3]], className="text-muted mb-0")
            ...
            if sensors_needing_retraining:
                recommendation_items.append(
                    dbc.ListGroupItem([
                        ...
                        html.P([eq['sensor_id'] for eq in sensors_needing_retraining[:3]], className="text-muted mb-0")
    ...
    def handle_training_operations(individual_clicks, all_clicks, interval_n,
                                 sensor_id, model_type, mode, progress_data, logs_data):
        ...
        try:
            ...
            if trigger_id == 'start-individual-training' and individual_clicks:
                if not sensor_id or not model_type:
                    return dbc.Alert("Please select sensor and model type", color="warning"), [], True
            ...
            # Start batch training
                ...
                progress = dbc.Progress([
                    dbc.Progress(value=0, id="batch-training-progress-bar", label="Starting batch training...")
                ], className="mb-3")

                logs = [
                    html.P(f"[{datetime.now().strftime('%H:%M:%S')}] Starting batch training for all sensors"),
                    html.P(f"[{datetime.now().strftime('%H:%M:%S')}] Training both anomaly detection and forecasting models")
                ]
            ...
            # Update progress
                ...
                if interval_disabled:
                    new_logs.append(
                        html.P(f"[{datetime.now().strftime('%H:%M:%S')}] Training completed successfully!",
