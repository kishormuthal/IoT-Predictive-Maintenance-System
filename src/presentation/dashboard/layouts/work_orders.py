                                        ...
                                        style_data_conditional=[
                                            {
                                                "if": {
                                                    "column_id": "priority",
                                                    "filter_query": "{priority} = CRITICAL",
                                                },
                        ...
                        [
                            ...
                            html.Div(
                                [
                                    html.Button(
                                        "Save Changes",
                                        id="save-work-order-btn",
                                        className="btn btn-primary",
                                    ),
                                    html.Button(
                                        "Cancel",
                                        id="cancel-work-order-btn",
...
def generate_status_chart(work_orders_df: pd.DataFrame) -> go.Figure:
    ...
    fig.update_layout(height=300, showlegend=True, margin=dict(t=20, b=20, l=20, r=20))
...
def generate_workload_chart(work_orders_df: pd.DataFrame) -> go.Figure:
    ...
    workload = (
        active_df.groupby("assigned_technician")
        .agg({"work_order_id": "count", "estimated_duration": "sum"})
        .reset_index()
    )
...
def calculate_statistics(work_orders_df: pd.DataFrame) -> Dict[str, int]:
    ...
    if work_orders_df.empty:
        return {"total": 0, "pending": 0, "in_progress": 0, "completed_today": 0}
