...
from src.presentation.dashboard.components.alert_action_manager import (
    alert_action_manager,
)
from src.presentation.dashboard.components.alert_system import AlertManager
from src.presentation.dashboard.components.detection_details_panel import (
    detection_details_panel,
)

# Import dashboard components
from src.presentation.dashboard.components.dropdown_manager import (
    dropdown_state_manager,
)

# Import Enhanced Dashboard Components
from src.presentation.dashboard.components.subsystem_failure_analyzer import (
    nasa_subsystem_analyzer,
)
from src.presentation.dashboard.components.threshold_manager import threshold_manager
...
class AnomalyMonitor:
    ...
    def create_layout(self) -> html.Div:
        ...
        return html.Div(
            [
                    ...
                    [
                        dbc.Col(
                            [
                                ...
                                html.P(
                                    "Phase 2: Advanced Analytics & Visualization with Subsystem Failure Analysis",
                ...
                dbc.Row(
                    [
                        dbc.Col([self._create_detection_status_card()], width=3),
                        dbc.Col([self._create_anomaly_rate_card()], width=3),
                        dbc.Col([self._create_model_status_card()], width=3),
                        dbc.Col([self._create_alert_trigger_card()], width=3),
                    ],
                    className="mb-4",
                ),
                        ...
                        dbc.Tabs(
                            [
                                ...
                                dbc.Tab(
                                    ...
                                    tab_id="subsystem-tab",
                                    active_tab_class_name="fw-bold",
                                ),
                                dbc.Tab(
                                    label="🔍 Detection Details",
                                    tab_id="detection-tab",
                                    active_tab_class_name="fw-bold",
                                ),
                                dbc.Tab(
                                ...
                                dbc.Tab(
                                    label="⚙️ Threshold Config",
                                    tab_id="thresholds-tab",
                                    active_tab_class_name="fw-bold",
                                ),
                            ],
                            id="main-tabs",
                                ...
                                dbc.Card(
                                    [
                                        ...
                                        dbc.CardBody(
                                            [
                                                dcc.Graph(
                                                    id="subsystem-correlation-chart",
                                                    style={"height": "400px"},
                                                )
                                            ]
                                        ),
    ...
    def _create_alert_management_tab_content(self) -> html.Div:
        ...
        return html.Div(
            [
                ...
                dbc.Row(
                    [dbc.Col([html.Div(id="alert-action-interface")], width=12)],
                    className="mb-4",
                ),
    ...
    def _create_anomaly_details(self) -> html.Div:
        ...
        return html.Div(
            [
                ...
                html.Div(
                    id="nasa-sensor-metrics",
                    children=self._create_nasa_sensor_metrics(),
                ),
                html.Hr(),
                # NASA Alert Actions
                ...
                dbc.ModalBody(
                    [
                        ...
                        dbc.Checklist(
                            ...
                            options=[
                                {
                                    "label": "Auto-create alerts for critical anomalies",
                                    "value": "auto_critical",
                                },
                                {
                                    "label": "Send notifications for high severity",
                                    "value": "notify_high",
                                },
                                {
                                    "label": "Log all anomalies to database",
                                    "value": "log_all",
                                },
                                {
    ...
    def _get_hierarchical_equipment_options(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for option in dropdown_options:
                dash_option = {"label": option.label, "value": option.value}
                if option.disabled:
            ...
            if status["is_streaming"]:
                return dbc.Alert(
                    [
                        ...
                        html.Span(
                            f" | {status['stream_stats'].get('total_updates', 0)} updates",
    ...
    def get_nasa_alert_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "severity_counts": alert_summary.get(
    ...
    def get_sensor_time_series_data(
        self,
        ...
        try:
            ...
            from src.data_ingestion.unified_data_controller import (
            ...
            if equipment_id and selected_sensors:
                try:
                    ...
                    nasa_data = unified_data_controller.get_latest_sensor_data(
                        limit=timesteps, equipment_filter=equipment_id
                    )
    ...
    def register_callbacks(self, app):
        ...
        def update_central_filters_and_sensors(
            ...
            logger.info(
                f"[DROPDOWN] UPDATE: equipment_id={equipment_id}, hierarchical={hierarchical_equipment_id}, filtered={selected_equipment}"
            )
        ...
        def handle_quick_sensor_selection(
            all_clicks,
            power_clicks,
            comm_clicks,
            clear_clicks,
            ...
            if button_id == "clear-sensor-selection":
                ...
                power_sensors = [
                    ...
                    if "power" in opt["label"].lower()
                    or "voltage" in opt["label"].lower()
                    or "current" in opt["label"].lower()
                    or "battery" in opt["label"].lower()
                ]
                ...
                comm_sensors = [
                    opt["value"]
                    for opt in (available_options or [])
                    if "comm" in opt["label"].lower()
                    or "signal" in opt["label"].lower()
                    or "antenna" in opt["label"].lower()
                    or "transmission" in opt["label"].lower()
                ]
        ...
        def update_metric_options(equipment_id, selected_sensors):
            ...
            try:
                if not equipment_id or not selected_sensors:
                    return [
                        {
                            "label": "Select equipment and sensor first",
                            "value": None,
                            "disabled": True,
                        }
                    ]
                ...
                if selected_sensor and selected_sensor != "ALL":
                    ...
                    logger.info(
                        f"[DROPDOWN] Metric options updated: {len(dash_options)} metrics for {equipment_id}::{selected_sensor}"
                    )
                    return dash_options
                else:
        ...
        try:
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    y=status_y_values,
                    mode="markers+text",
                    marker=dict(size=15, color=status_colors, symbol="circle"),
                    text=status_text,
