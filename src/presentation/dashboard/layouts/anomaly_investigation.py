...
def create_anomaly_investigation_layout() -> html.Div:
    ...
    return html.Div([
                    ...
                    dbc.Col([
                        ...
                        dcc.Dropdown(
                            ...
                            options=[
                                ...
                                {"label": "SMAP - Power System", "value": "smap_power"},
                                {"label": "SMAP - Communication", "value": "smap_comm"},
                                {"label": "MSL - Mobility Front", "value": "msl_mobility_f"},
                                {"label": "MSL - Power System", "value": "msl_power"},
                            ],
                            value="all"
                        )
                    ], width=3),
                    ...
                    dbc.Col([
                        ...
                        dcc.Dropdown(
                            ...
                            options=[
                                ...
                                {"label": "Contextual Anomaly", "value": "contextual"},
                                {"label": "Collective Anomaly", "value": "collective"},
                            ],
                            value="all"
                        )
                    ], width=3),
        ...
        html.Div(id="selected-anomaly-container", children=[
            dbc.Alert([
    ...
    return html.Div([
        ...
        dbc.Card([
            ...
            dbc.CardBody([
                dbc.Row([
                    dbc.Col([
                        html.P([html.Strong("Sensor: "), anomaly_data['sensor']], className="mb-1"),
                        html.P([html.Strong("Detected: "), anomaly_data['timestamp']], className="mb-1"),
                    ], width=4),
                    dbc.Col([
                        html.P([html.Strong("Anomaly Score: "), f"{anomaly_data['score']:.3f}"], className="mb-1"),
                        html.P([html.Strong("Confidence: "), f"{anomaly_data['confidence']*100:.1f}%"], className="mb-1"),
                    ], width=4),
                    dbc.Col([
                        html.P([html.Strong("Expected Range: "), f"{anomaly_data['expected_min']:.2f} - {anomaly_data['expected_max']:.2f}"], className="mb-1"),
                        html.P([html.Strong("Actual Value: "), f"{anomaly_data['actual_value']:.2f}"], className="mb-1"),
                    ], width=4),
                ])
            ])
        ], className="mb-4"),
        ...
        dbc.Tabs([
                ...
                dbc.Card([
                    dbc.CardBody([
                        ...
                        dbc.Row([
                            ...
                            dbc.Col([
                                html.H6("Statistical Analysis", className="mb-3"),
                                dcc.Graph(
                            ...
                            dbc.Col([
                                html.H6("Probability Distribution", className="mb-3"),
                                dcc.Graph(
            ...
            ], label="Sensor Analysis", tab_id="sensor-analysis"),
            ...
            dbc.Tab([
                dbc.Card([
                    dbc.CardBody([
                        ...
                        # Detailed factors
                        html.H6("Identified Root Causes:", className="mb-3"),
                        html.Div(id="root-cause-details", children=create_root_cause_details(anomaly_data))
                    ])
                ])
            ], label="Root Cause Analysis", tab_id="root-cause"),

            # Tab 3: Correlation Analysis
            dbc.Tab([
                dbc.Card([
                    dbc.CardBody([
                        html.H5("Multi-Sensor Correlation", className="mb-3"),
                        ...
                        dbc.Row([
                            ...
                            dbc.Col([
                                html.H6("Correlated Sensors", className="mb-3"),
                                html.Div(id="correlated-sensors", children=create_correlated_sensors_list(anomaly_data))
                            ], width=6)
                        ], className="mb-3"),
                        ...
                        html.H6("Time-Lagged Cross-Correlation", className="mb-3"),
                        dcc.Graph(
            ...
            ], label="Correlation Analysis", tab_id="correlation"),

            # Tab 4: Similar Anomalies
            dbc.Tab([
                dbc.Card([
                    dbc.CardBody([
                        html.H5("Historical Similar Anomalies", className="mb-3"),

                        # Similarity scores
                        html.Div(id="similar-anomalies", children=create_similar_anomalies_list(anomaly_data))
                    ])
                ])
            ], label="Similar Anomalies", tab_id="similar"),
                        ...
                        dbc.ListGroup([
                            dbc.ListGroupItem([
                                html.Div([
                                    ...
                                    html.H6("Immediate Action Required", className="mt-2"),
                                    html.P("Schedule maintenance inspection for power system within 4 hours"),
                                    dbc.Button("Create Work Order", color="warning", size="sm")
                                ])
                            ]),
                            ...
                            dbc.ListGroupItem([
                                html.Div([
                                    ...
                                    html.H6("Monitor Closely", className="mt-2"),
                                    html.P("Set up enhanced monitoring for correlated sensors (Communication, Thermal)"),
                                    dbc.Button("Enable Enhanced Monitoring", color="info", size="sm", outline=True)
                                ])
                            ]),
                            ...
                            dbc.ListGroupItem([
                                html.Div([
                                    ...
                                    html.H6("Alert Configuration", className="mt-2"),
                                    html.P("Configure predictive alerts for similar patterns"),
                                    dbc.Button("Configure Alerts", color="primary", size="sm", outline=True)
                                ])
                            ]),
                        ])
                    ])
                ])
            ], label="Recommendations", tab_id="recommendations"),

        ], id="anomaly-detail-tabs", active_tab="sensor-analysis")
    ])
        ...
        for sev in ["critical", "high", "medium", "low"]:
            ...
            if sev_anomalies:
                fig.add_trace(go.Scatter(
                    ...
                    text=[f"ID: {a['id']}<br>Sensor: {a['sensor']}<br>Score: {a['score']:.3f}" for a in sev_anomalies],
...
def create_anomaly_timeseries(anomaly_data: Dict) -> go.Figure:
    ...
    times = pd.date_range(
        end=pd.to_datetime(anomaly_data['timestamp']),
        periods=100,
        freq='1min'
    )
...
def create_statistical_breakdown(anomaly_data: Dict) -> go.Figure:
    ...
    anomaly_window = [anomaly_data['actual_value'], anomaly_data['actual_value'], 8, 45, anomaly_data['actual_value']]

    fig = go.Figure()
    fig.add_trace(go.Bar(name='Normal Period', x=categories, y=normal_values, marker_color='#3498db'))
    fig.add_trace(go.Bar(name='Anomaly Window', x=categories, y=anomaly_window, marker_color='#e74c3c'))
    ...
    for cause in causes:
        cards.append(
            dbc.Card([
                dbc.CardBody([
                    html.H6([
                        dbc.Badge(f"{cause['contribution']*100:.0f}%", color="warning", className="me-2"),
                        cause['factor']
                    ]),
                    html.P(cause['description'], className="mb-2"),
                    html.P([html.Strong("Evidence: "), cause['evidence']], className="mb-2 small"),
                    html.P([html.Strong("Recommended Action: "), cause['action']], className="mb-0 small text-primary")
                ])
            ], className="mb-2")
        )
        ...
        items.append(
            dbc.ListGroupItem([
                html.Div([
                    ...
                    dbc.Badge(f"r = {sensor['correlation']:.2f}", color="info", className="float-end"),
                ]),
        ...
        cards.append(
            dbc.Card([
                dbc.CardBody([
                    dbc.Row([
                        dbc.Col([
                            html.P([html.Strong("ID: "), html.Code(anom['id'])], className="mb-1"),
                            html.P([html.Strong("Date: "), anom['date']], className="mb-1"),
                        ], width=6),
                        dbc.Col([
                            html.P([
                                html.Strong("Similarity: "),
                                dbc.Badge(f"{anom['similarity']*100:.0f}%", color="success")
                            ], className="mb-1"),
                            dbc.Button("View Details", size="sm", color="info", outline=True)
                        ], width=6, className="text-end")
                    ]),
                    html.Hr(className="my-2"),
                    html.P([html.Strong("Resolution: "), anom['resolution']], className="mb-1 small"),
                    html.P([html.Strong("Outcome: "), anom['outcome']], className="mb-0 small text-success")
                ])
            ], className="mb-2")
        )
