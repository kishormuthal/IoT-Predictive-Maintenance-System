    ...
    return html.Div([
        ...
        dbc.Row([
            ...
            dbc.Col([
                dbc.ButtonGroup([
                    dbc.Button(
                        [html.I(className="fas fa-plus me-2"), "New Training Job"],
                ...
                dbc.Card([
                    dbc.CardBody([
                        html.Div([
                            ...
                            html.H3(id="training-active-jobs", children="0"),
                            html.P("Active Jobs", className="text-muted mb-0")
                ...
                dbc.Card([
                    dbc.CardBody([
                        html.Div([
                            ...
                            html.H3(id="training-queued-jobs", children="0"),
                            html.P("Queued Jobs", className="text-muted mb-0")
                        ], className="text-center")
                    ])
                ])
            ], width=3),
                ...
                dbc.Card([
                    dbc.CardBody([
                        html.Div([
                            ...
                            html.H3(id="training-completed-jobs", children="0"),
                            html.P("Completed (24h)", className="text-muted mb-0")
                        ], className="text-center")
                    ])
                ])
            ], width=3),
                ...
                dbc.Card([
                    dbc.CardBody([
                        html.Div([
                            ...
                            html.H3(id="training-failed-jobs", children="0"),
                            html.P("Failed (24h)", className="text-muted mb-0")
                        ], className="text-center")
                    ])
                ])
            ], width=3),
        ...
        dbc.Tabs([
            ...
            ], label="Active Jobs", tab_id="active-jobs"),
                            ...
                            dbc.Col([
                                ...
                                dcc.Dropdown(
                                    ...
                                    options=[
                                        ...
                                        {"label": "LSTM Predictor", "value": "lstm_predictor"},
                                        {"label": "LSTM Autoencoder", "value": "lstm_autoencoder"},
                                        {"label": "LSTM VAE", "value": "lstm_vae"},
            ...
            ], label="Resources", tab_id="resources"),

        ], id="training-tabs", active_tab="active-jobs"),
        ...
        dbc.Modal([
                ...
                dbc.Row([
                    dbc.Col([
                        ...
                        dcc.Dropdown(
                            ...
                            options=[
                                {"label": "SMAP (Soil Moisture Active Passive)", "value": "smap"},
                                {"label": "MSL (Mars Science Laboratory)", "value": "msl"},
                                {"label": "Combined Dataset", "value": "combined"},
                            ],
                ...
                dbc.Row([
                    dbc.Col([
                        dbc.Checklist(
                            ...
                            options=[
                                {"label": "Enable early stopping", "value": "early_stopping"},
                                {"label": "Enable MLflow tracking", "value": "mlflow"},
                                {"label": "Use GPU acceleration", "value": "gpu"},
                                {"label": "Save checkpoints", "value": "checkpoints"},
                            ],
                            value=["early_stopping", "mlflow", "checkpoints"],
                            inline=False
                        )
                    ], width=12)
                ])
            ...
            dbc.ModalFooter([
                dbc.Button("Cancel", id="new-training-cancel-btn", color="secondary"),
                dbc.Button("Start Training", id="new-training-start-btn", color="primary"),
            ])
        ], id="new-training-modal", size="lg", is_open=False),
...
def register_training_monitor_callbacks(app: dash.Dash):
    ...
    def update_training_stats(n_intervals, n_clicks):
        ...
        return len(active_jobs), len(queued_jobs), completed_count, failed_count, jobs_data
            ...
            job_card = dbc.Card([
                dbc.CardHeader([
                    dbc.Row([
                        ...
                        dbc.Col([
                            dbc.Badge(f"Epoch {job['epoch']}/{job['total_epochs']}", color="info", className="me-2"),
                ...
                dbc.CardBody([
                    # Progress bar
                    html.Div([
                        html.Label(f"Progress: {job['progress']}%", className="mb-2"),
                        dbc.Progress(
                            value=job['progress'],
                            striped=True,
                            animated=True,
                            color="success" if job['progress'] > 66 else "warning" if job['progress'] > 33 else "info",
                            className="mb-3"
                        ),
                    ]),
                    ...
                    dbc.Row([
                        dbc.Col([
                            html.P([html.Strong("Job ID: "), html.Code(job["id"])], className="mb-1"),
                            html.P([html.Strong("Current Loss: "), f"{job['current_loss']:.4f}"], className="mb-1"),
                        ], width=6),
                        dbc.Col([
                            html.P([html.Strong("Started: "), datetime.fromisoformat(job['started']).strftime('%H:%M:%S')], className="mb-1"),
                            html.P([html.Strong("Status: "), dbc.Badge("Running", color="success")], className="mb-1"),
                        ], width=6)
                    ]),
                    ...
                    # Actions
                    dbc.ButtonGroup([
                        dbc.Button([html.I(className="fas fa-eye me-2"), "View Logs"], size="sm", color="info", outline=True),
                        dbc.Button([html.I(className="fas fa-stop me-2"), "Stop"], size="sm", color="danger", outline=True),
                    ], className="mt-2")
                ])
            ], className="mb-3")
            ...
            queue_items.append(
                dbc.ListGroupItem([
                    dbc.Row([
                        ...
                        dbc.Col([
                            html.Span(f"Submitted: {datetime.fromisoformat(job['submitted']).strftime('%H:%M:%S')}", className="text-muted")
                        ], width=4),
                        dbc.Col([
                            dbc.Button("Cancel", size="sm", color="danger", outline=True)
    ...
    def update_training_history(status_filter, model_filter, time_range):
        ...
        for job in history_data:
            ...
            rows.append(html.Tr([
                ...
                html.Td([
                    dbc.ButtonGroup([
                        dbc.Button("Details", size="sm", color="info", outline=True),
                        dbc.Button("Logs", size="sm", color="secondary", outline=True),
                    ])
                ])
            ]))
