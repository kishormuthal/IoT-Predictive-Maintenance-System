...
class MaintenanceScheduler:
    ...
    def create_layout(self) -> html.Div:
        ...
        return html.Div(
            [
                ...
                dbc.Row(
                    [
                        dbc.Col([self._create_schedule_overview_card()], width=3),
                        dbc.Col([self._create_resource_utilization_card()], width=3),
                        dbc.Col([self._create_compliance_status_card()], width=3),
                        dbc.Col([self._create_cost_forecast_card()], width=3),
                    ],
                    className="mb-4",
                ),
                # View filters
    ...
    def _create_calendar_view(self) -> html.Div:
        ...
        return html.Div(
            [
                ...
                # Calendar grid
                html.Div(id="calendar-grid", children=[self._create_month_calendar()]),
            ]
        )
            ...
            for task in sample_tasks[day]:
                ...
                tasks.append(
                    html.Div(
                        [
                            html.I(
                                className=f"fas {self.maintenance_types[task['type']]['icon']} me-1",
                                style={"fontSize": "10px", "color": color},
                            ),
                            html.Span(
                                f"{task['count']} {task['type']}",
                                style={"fontSize": "11px"},
                            ),
                        ],
                    ...
                    [
                        html.Div(
                            [
                                ...
                                html.Small(
                                    f"{tech['workload']}%",
                                    className=f"text-{workload_color}",
                                ),
                            ],
                ...
                dbc.ModalBody(
                    [
                        ...
                        dbc.RadioItems(
                            ...
                            options=[
                                {
                                    "label": "Reschedule new task to next available slot",
                                    "value": "reschedule_new",
                                },
                                {
                                    "label": "Reschedule existing tasks",
                                    "value": "reschedule_existing",
                                },
                                {
                                ...
                                {
                                    "label": "Override and create anyway",
                                    "value": "override",
                                },
                            ],
                            value="reschedule_new",
    ...
    def create_gantt_chart(self, start_date: str, end_date: str) -> go.Figure:
        ...
        fig.update_layout(
            ...
            xaxis=dict(title="Date", type="date", range=[start_date, end_date]),
            yaxis=dict(
                title="Tasks",
                tickmode="array",
